...
export default function HistoricoDivergenciasPage() {
  ...
  const { data: historico = [], isLoading: loadingHist } = useQuery({
    ...
    enabled: !!selectedId,
    queryFn: async () => {
      const { data, error } = await supabase.from("divergencia_historico" as any).select("*").eq("divergencia_id", selectedId!).order("created_at", { ascending: true });
  ...
  const filtered = useMemo(() => {
    ...
    return finalizados.filter(d => d.nome_fornecedor.toLowerCase().includes(q) || d.codigo_fornecedor.toLowerCase().includes(q) || (d.requisicao_dc || "").toLowerCase().includes(q) || (d.nota_fiscal || "").toLowerCase().includes(q));
  }, [finalizados, busca]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3"><Archive className="h-6 w-6 text-muted-foreground" /><div><h1 className="text-2xl font-bold tracking-tight">Histórico</h1><p className="text-muted-foreground text-sm mt-1">Divergências concluídas</p></div></div>

      <div className="relative max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Buscar fornecedor, NF..." value={busca} onChange={e => setBusca(e.target.value)} className="pl-9" /></div>
      ...
      <Card className="border-0 shadow-sm overflow-hidden">
        {isLoading ? <div className="flex items-center justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : (
          <Table><TableHeader><TableRow className="bg-muted/50"><TableHead className="font-semibold">Data</TableHead><TableHead className="font-semibold">Loja</TableHead><TableHead className="font-semibold">Fornecedor</TableHead><TableHead className="font-semibold">Ocorrência</TableHead><TableHead className="font-semibold">NF Devolução</TableHead><TableHead className="font-semibold">Finalizado em</TableHead></TableRow></TableHeader>
            <TableBody>{filtered.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center py-12 text-muted-foreground">Nenhum registro</TableCell></TableRow> : filtered.map(d => (
      ...
      <Dialog open={timelineOpen} onOpenChange={setTimelineOpen}>
        <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
          ...
          {loadingHist ? <div className="flex items-center justify-center py-8"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div> : historico.length === 0 ? <p className="text-sm text-muted-foreground text-center py-6">Nenhum registro.</p> : (
